...
int utils::createListenFd(int port)
{
    ...
    if((listenFd = ::socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0)) == -1) {
        std::perror("[Utils::createListenFd] socket");
        return -1;
    }
    ...
    if(::setsockopt(listenFd, SOL_SOCKET, SO_REUSEADDR, (const void*)&optval, sizeof(int)) == -1) {
        std::perror("[Utils::createListenFd] setsockopt");
        return -1;
    }
    ...
    if(::bind(listenFd, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == -1) {
        std::perror("[Utils::createListenFd] bind");
        return -1;
    }

    // 开始监听，最大监听队列为LISTENQ
    if(::listen(listenFd, LISTENQ) == -1) {
        std::perror("[Utils::createListenFd] listen");
        return -1;
    }
...
int utils::setNonBlocking(int fd)
{
    ...
    if(flag == -1) {
        std::perror("[Utils::setNonBlocking] fcntl");
